...
(function($) {
    ...
    function test() {
        var userAns = $ans.val();
        correctAns = eval(firstNum + car + lastNum);

        return userAns == correctAns;
    }

    function checkBefore() {
        var score = eval(firstNum + car + lastNum);
        return score % 1 != 0;
    }
    ...
    function addFail() {
        fail++;
        $('.fail #fail').text(fail);
        showAlert('danger', 'The correct answer is: ' + correctAns)
    }
    ...
    function showAlert(type, msg) {
        ...
        $alert.addClass('alert-' + type).html(msg).addClass('in');
        lastType = type;

        alertTimer = setTimeout(function() {
            $alert.removeClass('in');
            if(lastType) {
                $alert.removeClass('alert-' + lastType);
            }
        }, 1500);
    }
    ...
    function updateTiming() {
        var current_time = timing.msToTime(timing.lap());
        $('#timing').text(current_time);
    }
